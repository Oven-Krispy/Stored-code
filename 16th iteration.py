...
# allows the use of the counter function

def compare_hands(players):
    ranked_hands = [(Hand(player.hand).rank_hand(),
    ...
    return f"{winner.name} wins with a {win_type} of high card {high_card}"
...
class Hand:
    ...
    def rank_hand(self):
        ...
        if straight and flush:
            return (8, self.high_card(ranks), "Straight Flush")  # Straight Flush
        if rank_counts[0][1] == 4:
            return (7, rank_counts[0][0], "Four of a Kind")  # Four of a Kind
        if rank_counts[0][1] == 3 and rank_counts[1][1] == 2:
            return (6, rank_counts[0][0], "Full House")  # Full House
        if flush:
            return (5, self.high_card(ranks), "Flush")  # Flush
        if straight:
            return (4, self.high_card(ranks), "Straight")  # Straight
        if rank_counts[0][1] == 3:
            return (3, rank_counts[0][0], "Three of a Kind")  # Three of a Kind
        if rank_counts[0][1] == 2 and rank_counts[1][1] == 2:
            return (2, rank_counts[0][0], "Two Pair")  # Two Pair
        if rank_counts[0][1] == 2:
            return (1, rank_counts[0][0], "One Pair")  # One Pair
        return (0, self.high_card(ranks), "High Card")  # High Card
...
class TexasHoldem:
    ...
    def display_available_funds(self):
      ...
      + str(self.initial_funds), True, (0, 0, 0))
    ...
    def display_pot_total(self):
      pot_text = self.small_font.render("Pot: "
      + str(self.pot_total), True, (226, 172, 8))
      # sets the font for the displaying of the pot
      self.screen.blit(pot_text, (400, 390))
      # displays the pot text
    
    def display_AI_player_funds(self):
      funds = 1000
      funds_text = self.small_font.render("Funds: "
      + str(funds), True, (226, 172, 8))
      # sets the text to be printed as the player's funds
      for count in range(2):
        self.screen.blit(funds_text, (30 + count * 630,
        140 + count * 10))
        # displays the left and right player's funds
        self.screen.blit(funds_text, (210 + count * 230,
        140))
    ...
    def display_all_bets(self):
      ...
      + str(bet), True, (226, 172, 8))
      # sets the text to be used for the player's bets
      self.screen.blit(bet_text, (640, 450))
      # displays the user's bet
      for count in range(2):
        self.screen.blit(bet_text, (30 + count * 630,
        170 + count * 10))
        # displays the left and right player's bets
        self.screen.blit(bet_text, (210 + count * 230,
        170))
        # displays the top two player's bets
    ...
    def play(self):
        ...
        while running:
          # keybinds in order to test display events 
            ...
            self.display_pot_total()
            # displays the pot total
            self.display_AI_player_funds()
            # displays the funds of the AI players
            self.display_all_bets()
            # displays the player's bets
            pygame.display.flip()
            clock.tick(120),
            # runs the display
            ...
            result = compare_hands(self.players)
            print(result)
        pygame.quit()
