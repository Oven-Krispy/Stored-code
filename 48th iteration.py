...
class TexasHoldem:
  ...
  def display_fixed_drawings(self):
    ...
    for count in range(2):
      ...
      (352 + count * 80, 32, 66, 106))
      # draws the back of the bot's cards
      pygame.draw.rect(self.screen, self.back_colour,
  ...
  def play(self):
    ...
    self.user_funds = 5100 - self.blinds
    self.bot_funds = 5100 - self.blinds
    self.user_bet = self.blinds
    self.bot_bet = self.blinds
    self.pot_total = self.user_bet + self.bot_bet
    # sets the initial values of the attributes
    
    while self.running:
        for event in pygame.event.get():
          if event.type == pygame.KEYDOWN:
            self.user_max_bet = self.user_funds
            self.bot_max_bet = self.bot_funds
            ...
            if event.key == pygame.K_e:
              ...
              while bet_input_active:
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_RETURN:
                            ...
                            if user_input <= "":
                              ...
                              self.user_bet = round_to_nearest_100(int(user_input))

                            self.user_funds -= self.user_bet
                            self.bot_funds -= int(self.bot_bet)
                            ...
                            self.match_bot_bet(user_input)
                            
                            self.update_display_to_clear()
                          ...
                          if is_integer(user_input):
                            self.user_bet = 0 if user_input == "" else int(user_input)
                            # removes the error when checking an 
                            # empty input
                            if self.user_bet > self.user_funds:
                              
                              user_input = str(self.user_funds)
                              self.user_bet = user_input
                              # sets any exceeding value to the user's 
                              # maximum value
                              
                            if int(user_input) > 100:
                              if int(self.user_bet) >= self.bot_max_bet:
                                self.bot_bet = self.bot_max_bet
                              else:
                                self.bot_bet = round_to_nearest_100(int(self.user_bet))
                ...
                self.display_updating_text(user_input)
                # displays the text the user writes
                

              self.pot_total += int(self.bot_bet) + int(self.user_bet)
  ...
  def update_display_to_clear(self):
    #self.screen.fill((0, 128, 0))
  ...
  def reset_round_variables(self):
    ...
    self.bot_bet = self.blinds
    self.pot_total = self.user_bet + self.bot_bet
    self.user_funds -= self.blinds
    self.bot_funds -= self.blinds
    # sets all of the bets and funds again
    self.game_round = -1
    self.community_cards_total = 0
    # resets what round of the game is next
  
  def display_updating_text(self, user_input):
    ...
    # corner
    bot_bet_text = self.small_font.render(f"Bet: {self.bot_bet}",
    True, self.yellow_text)
  ...
  def check_if_bust(self):
    if self.bot_funds <= 0:
      ...
      True, self.yellow_text)
      self.screen.blit(user_win_text, (50, 270))
      pygame.display.flip()
      time.sleep(3)
      self.running = False
      main()
    ...
    if self.user_funds <= 0:
      bot_win_text = self.font.render("You bust!",
      True, self.yellow_text)
      self.screen.blit(bot_win_text, (50, 270))
      pygame.display.flip()
      time.sleep(3)
      self.running = False
      main()

  def match_bot_bet(self, user_input):
    if user_input == "":
      pass
      ...
      while checking:
        if int(user_input) > self.user_max_bet:
          self.bot_bet = self.user_bet
          # bot matches the user's bet
          checking = False
          # check is ended
        if int(user_input) > self.bot_max_bet:
          self.bot_bet = self.bot_max_bet
          checking = False
        else:
          self.bot_bet = 100
  ...
  def iterate_round(self):
    ...
    if self.user_funds > 0:
      if self.game_round == 1:
        ...
        self.community_cards_total += 3
      elif self.game_round == 2:
        self.deal_community_card(1)
        # deals the turn card
        self.community_cards_total += 1
        ...
        if name == 'Bot':
          ...
          self.user_funds += self.pot_total
          self.pot_total = 0
        ...
        for j in range(2):
          self.draw_card(self.players[1].hand[j],
          350 + j * 80, 30, face_up = True)
          # draws the user's two cards
      ...
      leftover_community_cards = 5 - self.community_cards_total
      self.deal_community_card(leftover_community_cards)
