...
class TexasHoldem:
  ...
  def play(self):
    ...
    while running:
      ...
      while betting:
        for count in range(3):
        # repeats for each betting round
          if count == 0:
            initial_user_funds = 1000
            initial_user_bet = 0
            initial_AI_funds = 100
            initial_AI_bet = 0
            initial_pot_total = 0
            # sets the initial display values 
            self.display_fixed_drawings()
            # draws all the fixed items on the screen
            self.display_fixed_text(initial_user_funds,
            initial_user_bet, initial_AI_funds, initial_AI_bet,
            initial_pot_total)
            # writes the text with the initial values as 
            # placeholders
            self.display_community_cards()
            # displays the community cards onto the table
            pygame.display.flip()
            clock.tick(60)
            # loads the display
          ...
          if count == 0:
            ...
            AI_bet, pot_total)
            # draws the items and text on the screen
            self.deal_community_card(3)
            # deals and displays the cards for the flop
            pygame.display.flip()
            clock.tick(60)
            # loads the display
          ...
          if count == 1:
            ...
            AI_bet = 150
            AI_funds -= AI_bet
            # sets the AI player's bet and takes it from their 
            # funds
            pot_total += AI_bet + user_bet
          ...
          if count == 2:
            user_bet = int(input("enter your bet: "))
            user_funds -= user_bet
            # lets the user place their bet and removes it
            # from their funds
            
            AI_bet = 150
            AI_funds -= AI_bet
            # sets and removes the AI player's bet from their
            # funds
            ...
            # draws the items and text onto the screen
            self.deal_community_card(1)
            # deals and displays the river card
            pygame.display.flip()
            clock.tick(60)
            # loads the display
            betting = False
            # ends the betting loop
      
      user_funds += pot_total
      # gives the pot funds to the winner
      pot_total = 0
      user_bet = 0
      AI_bet = 0
      # sets the variables back to zero
          
      self.display_fixed_drawings()
      self.display_fixed_text(user_funds, user_bet, AI_funds,
      AI_bet, pot_total)  
      self.display_community_cards()
      # draws the items, writes the text and displays the cards
      # onto the screen
      pygame.display.flip()
      clock.tick(60)
      # loads the display
