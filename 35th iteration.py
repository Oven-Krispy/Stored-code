...
class TexasHoldem:
  ...
  def play(self):
    ...
    betting = True
    round_starting = True
    # sets the three game loops to run
    clock = pygame.time.Clock()
    ...
    while running:
        ...
        if bot_funds <= 0:
          user_win_text = self.small_font.render("Bot bust!",
          True, self.yellow_text)
          self.screen.blit(user_win_text, (640, 300))
          
        elif user_funds <= 0:
          bot_win_text = self.small_font.render("You bust!",
          True, self.yellow_text)
          self.screen.blit(bot_win_text, (640, 300))
        else:
          pass
                ...
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_RETURN:
                            ...
                            self.display_fixed_text(user_funds,
                            bot_funds, pot_total)
